...
class sequence_in : public uvm::uvm_sequence<REQ,RSP>
{
 public:
  char ch_in;
 
  sequence_in( const std::string& name ) : uvm::uvm_sequence<REQ,RSP>( name )
  {
    std::cout << sc_core::sc_time_stamp() << ": constructor " << name << std::endl;
    uvm_sequence_base::set_automatic_phase_objection(1);
  }
  
  UVM_OBJECT_PARAM_UTILS(sequence_in<REQ,RSP>);
  ...
  void body()
  {
    ...
    UVM_INFO(this->get_name(), "Starting sequence_in", uvm::UVM_MEDIUM);
    ...
    for(int i = 0; i < 10; i++) // Each sequence will send 10 packets.
    {
      ...
      get_address(ch_in, &x, &y); // To set the packet destination address, according to the DUT's rules.
    ...
    UVM_INFO(this->get_name(), "Finishing sequence_in", uvm::UVM_MEDIUM);
  }
...
template <typename REQ = uvm::uvm_sequence_item, typename RSP = REQ>
class sequence_l_in : public sequence_in<REQ,RSP>
{
 public:
 
 sequence_l_in<REQ,RSP>( const std::string& name ) : sequence_in<REQ,RSP>( name )
  {
    sequence_in<REQ,RSP>::ch_in = 'L';
  }
...
template <typename REQ = uvm::uvm_sequence_item, typename RSP = REQ>
class sequence_n_in : public sequence_in<REQ,RSP>
{
 public:
 
 sequence_n_in<REQ,RSP>( const std::string& name ) : sequence_in<REQ,RSP>( name )
  {
    sequence_in<REQ,RSP>::ch_in = 'N';
  }
...
template <typename REQ = uvm::uvm_sequence_item, typename RSP = REQ>
class sequence_e_in : public sequence_in<REQ,RSP>
{
 public:
 
 sequence_e_in<REQ,RSP>( const std::string& name ) : sequence_in<REQ,RSP>( name )
  {
    sequence_in<REQ,RSP>::ch_in = 'E';
  }

};
// Class sequence to send packets to input channel S.
template <typename REQ = uvm::uvm_sequence_item, typename RSP = REQ>
class sequence_s_in : public sequence_in<REQ,RSP>
{
 public:
 
 sequence_s_in<REQ,RSP>( const std::string& name ) : sequence_in<REQ,RSP>( name )
  {
    sequence_in<REQ,RSP>::ch_in = 'S';
  }
...
template <typename REQ = uvm::uvm_sequence_item, typename RSP = REQ>
class sequence_w_in : public sequence_in<REQ,RSP>
{
 public:
 
 sequence_w_in<REQ,RSP>( const std::string& name ) : sequence_in<REQ,RSP>( name )
  {
    sequence_in<REQ,RSP>::ch_in = 'W';
  }
