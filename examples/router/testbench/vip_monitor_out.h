#ifndef VIP_MONITOR_OUT_H_
#define VIP_MONITOR_OUT_H_
...
class vip_monitor_out : public uvm::uvm_monitor
{
 public:
  ...
  vip_monitor_out(uvm::uvm_component_name name) : uvm_monitor(name), item_collected_port("item_collected_port"), router_ch_vif(0), checks_enable(false), coverage_enable(false)
  {}

  UVM_COMPONENT_UTILS(vip_monitor_out);
  ...
  void run_phase( uvm::uvm_phase& phase )
  {
    ...
    for(int i = 0; true; i++) // Monitor DUT's output channels continuously until the end of the test (defined by the class test).
    {
      for(int j = 0; j < 4; j++) { // To read the 4 flits of each packet.
        ...
        if(j == 0) { // If the first flit was just sent from the DUT:
          packet_info[i][0] = int(sc_core::sc_time_stamp().value())/1000; // Register the time that it was received.
        ...
        if(j == 3) // If the last flit was just sent by the DUT:
          packet_info[i][1] = int(sc_core::sc_time_stamp().value())/1000; // Register the time that it was captured.
        router_ch_vif->ack.write("1"); // Write 1 to ack to indicate that the flit was read.
        while(router_ch_vif->val.read() != "0") // Waits valid return to 0.
          sc_core::wait(clk_i->posedge_event());
        router_ch_vif->ack.write("0"); // Write 0 to ack to complete the handshake.
      }
      std::cout << sc_core::sc_time_stamp() << ": " << name() << " received " << std::endl;
      item_collected_port.write(p); // After the flits received from the DUT are successfully read, the packet can be sent to the scoreboard.

      if(checks_enable)
        std::cout << "no checks yet" << std::endl;
      if(coverage_enable)
        std::cout << "no checks yet" << std::endl;
    }
  }
