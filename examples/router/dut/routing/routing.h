...
struct routing: sc_module{
    ...
    sc_signal<sc_uint<32>> x_id_p0_fault_target_from_in, y_id_p0_fault_target_from_in; // Created by the Converter tool to get and send faults to an sc_in.
    sc_in<sc_bv<1>> rst_i;
...
//    sc_signal<ic_fsm> state;
    ...
    void logic(){     
        x_id_p0_fault_target_from_in.write(x_id_p0.read()); // Created by the Converter tool to get and send faults to an sc_in.
        y_id_p0_fault_target_from_in.write(y_id_p0.read()); // Created by the Converter tool to get and send faults to an sc_in.
...
//            switch (state)
            ...
            case s0:
                if(frame_i.read()=="1" and rok_i.read()=="1"){
                    ...
                    //cout << x_id_p0_fault_target_from_in << "|" << y_id_p0 << endl; // Created by the Converter tool to get and send faults to an sc_in.
                    if(x_dst_v != x_id_p0_fault_target_from_in){ // Created by the Converter tool to get and send faults to an sc_in.
                        if(x_dst_v > x_id_p0_fault_target_from_in){ // Created by the Converter tool to get and send faults to an sc_in.
                            state.write(s1);
                        }else {
                            state.write(s2);
                    ...
                    }else if(y_dst_v != y_id_p0_fault_target_from_in){ // Created by the Converter tool to get and send faults to an sc_in.

                        if(y_dst_v > y_id_p0_fault_target_from_in){ // Created by the Converter tool to get and send faults to an sc_in.
                            state.write(s3);
                        }else{
                            state.write(s4);
                        }
